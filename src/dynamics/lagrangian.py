import sympy as smp
from src.kinematics.jacobian import link_jacobian
...
def mass_matrix(T_list, joints, theta_syms):
    ...
    for i in range(n):
        J = link_jacobian(T_list, joints, i)
        m = joints[i]["mass"]
...
def potential_energy(T_list, joints):
...
def gravity_vector(V, theta_syms):
...
def coriolis_matrix(M, theta_syms, theta_dot_syms):
